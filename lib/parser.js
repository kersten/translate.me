...
module.exports = (function () {
    ...
    function extract (root, file, options, cb) {
        fs.readFile(file, function (err, data) {
            var placeholders = data.toString().match(/\{\{i18n[\s]+"(.*?)"(?:[\s]+"(.*?)")?(.*)?\}\}/g);
            ...
            placeholders = data.toString().match(/_\.translate\((.|[\r\n])*?((\\.|[^\\"])*?)\);/g);
            ...
            if (placeholders) {
                async.map(_.uniq(placeholders), function (placeholder, cb) {
                    ...
                    i18nString.update({locale: options.nativeLanguage, path: namespace, key: key}, {
                        locale: options.nativeLanguage,
                        path: namespace,
                        key: key,
                        value: ""
