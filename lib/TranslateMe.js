...
TranslateMe.prototype.registerRoutes = function(app, enableAdminUI) {
        ...
        switch(req.param('format')) {
            case 'require':
                res.sendData = function(object) {
                    res.type(express.mime.types.json).end("define(function() {return" + JSON.stringify(object) + "})");
                };
                next();
                break;
            case 'json':
    ...
    app.get(apiRoot + '/locales/master', function(req, res) {
        res.sendData(self.masterLocale);
    });
    ...
    app.get(apiRoot + "/keys", function(req, res) {
        self.translationStore.getTranslations({locale: masterLocale}, function(err, translations) {
            var keys = [];
            if(!err) {
                ...
                res.send(500, "Sorry. Couldn't retrieve keys. Here's the error: " + JSON.stringify(err))
            }
        });
    });
}
