...
TranslateMe.prototype.registerRoutes = function(app, enableAdminUI) {
    ...
    app.get('/i18n/translations', function(req, res) {
        var locale = req.param('locale'), // One or more locales.
            path = req.param('path');
        ...
        async.parallel({
            ...
            requestedTranslations: function(callback) {
                var query;
                if(!_.isEqual(locale, [l.Locale['default'].toString()])) {
    ...
    app.post('/i18n/translations', function(req, res) {
        ...
        update = _.clone(conditions);
        if(value) {
            update.value = value;
        }

        TranslationModel.findOneAndUpdate(conditions, update, {
            new: true,
            upsert: true
        }, function(err, model) {
            if(!err) {
