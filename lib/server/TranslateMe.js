...
TranslateMe.prototype.registerRoutes = function(app, enableAdminUI) {
        ...
        app.post(root + '/admin/search', function (req, res) {
            ...
            query.exec(function (err, models) {
                ...
                _.each(models, function(model) {
                    _.each(model.toOldModel(locales, true), function(oldModel) {
    ...
    app.get(apiRoot + '/masters', function(req, res) {
        TranslationModel.find(function(err, masterTranslations) {
            if(!err && masterTranslations) {
                var result = [];
                _.each(masterTranslations, function(masterTranslation) {
                    result.push(_.pick(masterTranslation, 'namespace', 'key'));
                });
                res.sendData(result);
            } else {
...
TranslateMe.prototype.getTranslator = function(ready) {
    ...
    if(this.translator === undefined) {
        ...
        TranslationModel.find(function(err, translationModels) {
            if(!err && translationModels) {
                translations = translationModels;
        ...
        this.translator = new Translator({
            get: function(locale, namespace, key) {
                var masterTranslation, translation;
                if(!_.isEmpty(translations)) {
                    masterTranslation = _.findWhere(translations, {
                        namespace: namespace,
                        key: key
                    })
                    if(masterTranslation) {
                        translation = _.findWhere(masterTranslation.translations, {
                            locale: locale
                        });
                        if(translation) {
            ...
            /**
             * Checks whether namespace and the key is already registered as a master.
             *
             * @param {string} namespace
             * @param {string} key
             * @returns {boolean} true if the master exists, otherwise false
             */
            hasMaster: function(namespace, key) {
                return _.findWhere(translations, {
                    namespace: namespace,
                    key: key
                }) !== undefined;
            },
            ...
            createMaster: function(namespace, key) {
                TranslationModel.getTranslationModel({
                    ...
                    if(!err && model) {
                        model.save(function(err, model) {
                            if(!err && model) {
                                if(!_.findWhere(translations, {_id: model._id})) {
                                    translations.push(model);
                                }
                            } else {
                                // TODO Proper logging
