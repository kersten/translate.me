var _ = require("underscore")._,
    ...
    Translator = require('../translator.umd.js'),
    mongoose = require("mongoose"),
    TranslationModel = mongoose.model("translation", require('./TranslationSchema')),
    helpers = require('../handlebars-helpers.umd.js'),
...
function TranslateMe (mongoURL, masterLocale, translatableLocales) {
    ...
    } else if (!masterLocale) {
        throw "Please pass a locale.";
    } else if (!translatableLocales) {
 ...
 * @param {Object} definition of mime types
 * @param {Function} [callback] a function which will be called, when the operation finished
 */
TranslateMe.prototype.generateDefaultTranslations = function(directories, projectDirectory, mimeTypes, callback) {
    "use strict";
    ...
    if (_.isFunction(mimeTypes) && callback === undefined) {
        callback = mimeTypes;
        mimeTypes = {
            'text/angular': ['html'],
    ...
    TranslationModel.ensureIndexes(function(err) {
        ...
        if (!err) {
            async.mapSeries(directories, function(directory, done) {
                index.build(directory, projectDirectory, mimeTypes, function(err, translations) {
                    if (!err) {
