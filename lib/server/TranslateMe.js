var _ = require("underscore")._,
    ...
    HandlebarsHelpers = require('../shared/handlebars-helpers.umd.js'),
    root = "/translate.me", apiRoot = root + "/api/:format",
    locales_list = require('fs').readFileSync(__dirname + '/../data/countrylist.csv').toString().replace(/^\uFEFF/, '').split("\n");
...
TranslateMe.prototype.getTranslator = function(ready) {
    ...
    if(this.translator === undefined) {
        // Cache translations
        this.translationStore.getTranslations({}, function(err, docs) {
            if(!err) {
                // Create an array which groups the translations in the following structure: [local][namespace][key]
        ...
        this.translator = new Translator({
            ...
            createMaster: function(namespace, key) {
                that.translationStore.createMetaTranslation({
                    namespace: namespace,
                    key: key
                }, 'dynamic');
            }
...
/**
 * Provides block helpers to support namespace and translation blocks in templates.
 *
 * @returns {{namespace: *, translation: *}}
 */
TranslateMe.prototype.HandlebarsBlockHelpers = function() {
    var that = this;
    return {
        namespace: HandlebarsHelpers.createNamespaceBlockHelper(),
        translation: HandlebarsHelpers.createTranslationBlockHelper(that.getTranslator())
    }
}
...
TranslateMe.prototype.registerRoutes = function(app, enableAdminUI) {
    ...
    app.get(apiRoot + '/locales_iso_list', function(req, res) {
        var locales = [];
        _.each(locales_list, function(locale) {
            if (locale) {
                locale = locale.toString().split(';');
                locales.push({
                    iso: locale[1].toLowerCase(),
                    name: locale[0]
                });
            }
        })
